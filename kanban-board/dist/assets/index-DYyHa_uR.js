import Lt from"https://cdn.jsdelivr.net/npm/mermaid@11/dist/mermaid.esm.min.mjs";(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const o of document.querySelectorAll('link[rel="modulepreload"]'))s(o);new MutationObserver(o=>{for(const r of o)if(r.type==="childList")for(const d of r.addedNodes)d.tagName==="LINK"&&d.rel==="modulepreload"&&s(d)}).observe(document,{childList:!0,subtree:!0});function t(o){const r={};return o.integrity&&(r.integrity=o.integrity),o.referrerPolicy&&(r.referrerPolicy=o.referrerPolicy),o.crossOrigin==="use-credentials"?r.credentials="include":o.crossOrigin==="anonymous"?r.credentials="omit":r.credentials="same-origin",r}function s(o){if(o.ep)return;o.ep=!0;const r=t(o);fetch(o.href,r)}})();Lt.initialize({startOnLoad:!1,theme:"dark",themeVariables:{primaryColor:"#334155",primaryBorderColor:"#60a5fa",primaryTextColor:"#f1f5f9",lineColor:"#64748b",secondaryColor:"#1e293b",tertiaryColor:"#0f172a"}});window.__mermaid=Lt;const Ft="modulepreload",Vt=function(e){return"/"+e},ft={},zt=function(n,t,s){let o=Promise.resolve();if(t&&t.length>0){let d=function(i){return Promise.all(i.map(u=>Promise.resolve(u).then(h=>({status:"fulfilled",value:h}),h=>({status:"rejected",reason:h}))))};document.getElementsByTagName("link");const c=document.querySelector("meta[property=csp-nonce]"),a=(c==null?void 0:c.nonce)||(c==null?void 0:c.getAttribute("nonce"));o=d(t.map(i=>{if(i=Vt(i),i in ft)return;ft[i]=!0;const u=i.endsWith(".css"),h=u?'[rel="stylesheet"]':"";if(document.querySelector(`link[href="${i}"]${h}`))return;const m=document.createElement("link");if(m.rel=u?"stylesheet":Ft,u||(m.as="script"),m.crossOrigin="",m.href=i,a&&m.setAttribute("nonce",a),document.head.appendChild(m),u)return new Promise((g,f)=>{m.addEventListener("load",g),m.addEventListener("error",()=>f(new Error(`Unable to preload CSS for ${i}`)))})}))}function r(d){const c=new Event("vite:preloadError",{cancelable:!0});if(c.payload=d,window.dispatchEvent(c),!c.defaultPrevented)throw d}return o.then(d=>{for(const c of d||[])c.status==="rejected"&&r(c.reason);return n().catch(r)})},G=[{key:"todo",label:"Requirements",icon:"📋"},{key:"plan",label:"Plan",icon:"🗺️"},{key:"plan_review",label:"Review Plan",icon:"🔍"},{key:"impl",label:"Implement",icon:"🔨"},{key:"impl_review",label:"Review Impl",icon:"📝"},{key:"test",label:"Test",icon:"🧪"},{key:"done",label:"Done",icon:"✅"}],Wt={plan:"Planning",plan_review:"Plan Review",impl:"Implementing",impl_review:"Impl Review",test:"Testing"},De="kanban-auth-token",Xe="kanban-current-view",It="kanban-mobile-board-columns",Kt=3e4,Jt=10,Yt=10,Ct="kanban-summary-cache",Gt={board:3e4,full:6e4},St=window.matchMedia("(max-width: 768px)");function Zt(){!("serviceWorker"in navigator)||!(window.location.protocol==="https:"||window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1")||window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").catch(n=>{console.warn("Service worker registration failed",n)})})}function Xt(e){return e==="board"||e==="list"||e==="chronicle"||e==="graph"}function Qt(e){return G.some(n=>n.key===e)}function en(){try{const e=localStorage.getItem(It);if(!e)return new Set;const n=JSON.parse(e);return Array.isArray(n)?new Set(n.filter(t=>typeof t=="string"&&Qt(t))):new Set}catch{return new Set}}let E=localStorage.getItem("kanban-project"),D=localStorage.getItem("kanban-category"),qe=[];const Ee=new Map;let gt=!1,He=!1,T=St.matches,C=Xt(localStorage.getItem(Xe))?localStorage.getItem(Xe):T?"list":"board",ue="",ee=localStorage.getItem("kanban-sort")||"default",N=localStorage.getItem("kanban-hide-old")==="true",F=localStorage.getItem(De)||"",V=!1,A=!1,Ze=!1,Le=!T,te=en(),B=null,be=null,ne=null;const $e=new Map,de=new Map,we=new Map;let Q=null,re=null;function pe(e,n="default"){const t=document.getElementById("auth-message");t.textContent=e,t.classList.remove("error","success"),n!=="default"&&t.classList.add(n)}function J(){const n=[document.getElementById("modal-overlay"),document.getElementById("add-card-overlay"),document.getElementById("auth-overlay")].some(t=>t&&!t.classList.contains("hidden"));document.body.classList.toggle("overlay-open",n)}function je(){const e=document.getElementById("auth-btn");if(e){if(!V){e.textContent="Open",e.title="Board access is open in this environment";return}e.textContent=A?"Private":"Locked",e.title=A?"Shared token configured for this browser":"Shared token required"}}function ce(e="Enter the shared access token to load the board.",n="default"){A=!1,document.getElementById("auth-overlay").classList.remove("hidden"),J();const t=document.getElementById("auth-token-input");t.value=F,pe(e,n),je(),setTimeout(()=>t.focus(),0)}function _e(){document.getElementById("auth-overlay").classList.add("hidden"),J(),je()}function jt(e){F=e.trim(),F?localStorage.setItem(De,F):localStorage.removeItem(De)}function et(){document.body.classList.toggle("mobile-shell",T),document.body.classList.toggle("mobile-toolbar-open",!T||Le);const e=document.getElementById("toolbar-mobile-toggle");if(e){const n=!T||Le;e.hidden=!T,e.setAttribute("aria-expanded",String(n)),e.textContent=n?"Hide Filters":"Show Filters"}}function tn(e){T=e,T||(Le=!0),et(),A&&C==="board"&&k()}function _t(){return window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1"}function tt(){be!==null&&(window.clearInterval(be),be=null)}function nt(e=E,n="full"){return`${e||"__all__"}::${n}`}function Ue(e=E,n="full"){return`${Ct}::${nt(e,n)}`}function nn(e=E,n="full"){try{const t=localStorage.getItem(Ue(e,n));if(!t)return null;const s=JSON.parse(t);return!s||typeof s.fetchedAt!="number"||!s.board?null:Date.now()-s.fetchedAt>Gt[n]?(localStorage.removeItem(Ue(e,n)),null):s}catch{return null}}function an(e,n,t,s){try{const o={fetchedAt:Date.now(),etag:s,board:t};localStorage.setItem(Ue(e,n),JSON.stringify(o))}catch{}}function z(e=E,n){const t=n?[n]:["board","full"];for(const s of t){const o=nt(e,s);$e.delete(o),de.delete(o),we.delete(o);try{localStorage.removeItem(Ue(e,s))}catch{}}e===E&&(B=null,ne=null)}function at(e={}){if($e.clear(),de.clear(),we.clear(),B=null,ne=null,e.persisted)try{const n=[];for(let t=0;t<localStorage.length;t+=1){const s=localStorage.key(t);s!=null&&s.startsWith(`${Ct}::`)&&n.push(s)}n.forEach(t=>localStorage.removeItem(t))}catch{}}function Bt(){F="",localStorage.removeItem(De);const e=document.getElementById("auth-token-input");e&&(e.value="")}async function sn(){const e=new Headers;F&&e.set("X-Kanban-Auth",F);const t=await(await fetch("/api/auth/session",{method:"GET",headers:e,credentials:"same-origin"})).json().catch(()=>({}));return{authenticated:!!t.authenticated,authRequired:!!t.authRequired,mode:t.mode,source:t.source??null,reason:t.reason??null,error:t.error??null}}async function Tt(e){const n=e.trim(),t=await fetch("/api/auth/session",{method:"POST",headers:{"X-Kanban-Auth":n},credentials:"same-origin"}),s=await t.json().catch(()=>({}));if(!t.ok){const o=s.reason==="invalid_token"?"Shared token is invalid.":s.reason==="token_hash_missing"?"Server auth is not configured yet.":"Board authentication failed.";throw new Error(o)}jt(n),V=!!s.authRequired,A=!0,_e(),je()}async function on(){await fetch("/api/auth/session",{method:"DELETE",credentials:"same-origin"}).catch(()=>{}),tt(),at({persisted:!0}),Bt(),A=!V,je()}async function _(e,n={},t=!1){const s=new Headers(n.headers||{});F&&!s.has("X-Kanban-Auth")&&s.set("X-Kanban-Auth",F);const o=await fetch(e,{...n,headers:s,credentials:"same-origin"});if((o.status===401||o.status===403||o.status===503)&&!t){const r=await o.clone().json().catch(()=>({}));V=!0,A=!1,r.reason==="invalid_token"&&Bt();const d=r.reason==="invalid_token"?"Stored token was rejected. Enter a valid shared token.":r.reason==="token_hash_missing"?"Server auth hash is not configured yet.":"Shared token is required for this board.";throw ce(d,"error"),new Error(r.error||d)}return o}async function st(e=E){const n=e?`?project=${encodeURIComponent(e)}`:"",t=new Headers;ne&&t.set("If-None-Match",ne);const s=await _(`/api/board/version${n}`,{headers:t});return s.status===304?B?null:(ne=null,st()):(ne=s.headers.get("ETag"),s.json())}function rn(e){return e==="board"?C==="board"&&!Qe():C==="list"||C==="chronicle"||C==="board"&&Qe()}function Qe(){return ue.trim().length>0}function cn(e,n,t,s){if(!A||we.has(n))return;const o=(async()=>{try{const r=await st(s);if(!r)return;if(t&&r.version===t){B=r.version;return}z(s,e),await me(e,{bypassTtl:!0,projectOverride:s}),E===s&&rn(e)&&k()}catch{}finally{we.delete(n)}})();we.set(n,o)}async function me(e="full",n={}){const t=n.projectOverride===void 0?E:n.projectOverride,s=["summary=true"];t&&s.unshift(`project=${encodeURIComponent(t)}`),e==="board"&&s.push("compact=board",`todo_limit=${Jt}`,`done_limit=${Yt}`);const o=`?${s.join("&")}`,r=nt(t,e);if(!n.bypassTtl){const h=nn(t,e);if(h)return $e.set(r,h.board),h.etag&&de.set(r,h.etag),B=h.board.version||B,cn(e,r,h.board.version||null,t),h.board}const d=new Headers,c=de.get(r);c&&d.set("If-None-Match",c);const a=await _(`/api/board${o}`,{headers:d});if(a.status===304){const h=$e.get(r);return h?(B=h.version||B,h):(de.delete(r),me(e,{bypassTtl:!0}))}const i=await a.json(),u=a.headers.get("ETag");return u&&de.set(r,u),$e.set(r,i),an(t,e,i,u),B=i.version||B,i}function ln(){_t()||be!==null||(be=window.setInterval(async()=>{if(!A||He)return;const e=!document.getElementById("modal-overlay").classList.contains("hidden"),n=!document.getElementById("add-card-overlay").classList.contains("hidden");if(!(e||n))try{const t=await st();if(!t)return;if(!B){B=t.version;return}t.version!==B&&(B=t.version,k())}catch{V&&!A&&tt()}},Kt))}function At(){if(_t()){tt(),Ot();return}ln()}function dn(){const e=new URL(window.location.href),n=e.searchParams.get("auth")||e.searchParams.get("token");n&&(jt(n),e.searchParams.delete("auth"),e.searchParams.delete("token"),window.history.replaceState({},"",e.toString()))}async function pn(){if(dn(),F)try{return await Tt(F),!0}catch(n){return ce(n instanceof Error?n.message:"Board authentication failed.","error"),!1}const e=await sn();return V=e.authRequired,A=e.authenticated||!e.authRequired,e.authRequired&&!e.authenticated?(ce("Enter the shared access token to load the board."),!1):(_e(),!0)}function Ve(e){return e==="high"?"high":e==="medium"?"medium":e==="low"?"low":""}function kt(){localStorage.setItem(It,JSON.stringify([...te]))}function un(e){if(!T||te.size>0)return;const n=G.filter(t=>t.key==="todo"||t.key==="impl"||t.key!=="done"&&e[t.key].length>0).map(t=>t.key);te=new Set(n.length>0?n:["todo"]),kt()}function mn(e){return!T||ue.trim()?!0:te.has(e)}function Ne(e){var n;return((n=G.find(t=>t.key===e))==null?void 0:n.label)||e}function hn(e,n){return e===1?{todo:["impl"],impl:["done"],done:[]}[n]||[]:e===2?{todo:["plan"],plan:["impl","todo"],impl:["impl_review"],impl_review:["done","impl"],done:[]}[n]||[]:{todo:["plan"],plan:["plan_review","todo"],plan_review:["impl","plan"],impl:["impl_review"],impl_review:["test","impl"],test:["done","impl"],done:[]}[n]||[]}async function fn(e,n){if(!n||n===e.status)return;const t=await _(`/api/task/${e.id}?project=${encodeURIComponent(e.project)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({status:n})});if(!t.ok){const s=await t.json().catch(()=>({}));Ae(s.error||"Failed to move task");return}z(e.project),Te()}function ve(e){return e?Date.now()-new Date(e).getTime()>4320*60*1e3:!1}function Rt(e,n){return ee==="default"?n==="done"?[...e].sort((t,s)=>{const o=(s.completed_at||"").localeCompare(t.completed_at||"");return o!==0?o:t.rank-s.rank||t.id-s.id}):[...e].sort((t,s)=>s.rank-t.rank||s.id-t.id):[...e].sort((t,s)=>ee==="created_asc"?t.created_at.localeCompare(s.created_at):ee==="created_desc"?s.created_at.localeCompare(t.created_at):ee==="completed_desc"?(s.completed_at||"").localeCompare(t.completed_at||""):0)}function ze(){const e=ue.toLowerCase().replace(/^#/,""),n=e.length>0||N;document.body.classList.toggle("mobile-board-search",C==="board"&&T&&e.length>0),C==="board"?(document.querySelectorAll(".card").forEach(t=>{const s=!e||(()=>{var i,u,h,m;const r=t.dataset.id||"",d=((u=(i=t.querySelector(".card-title"))==null?void 0:i.textContent)==null?void 0:u.toLowerCase())||"",c=((m=(h=t.querySelector(".card-desc"))==null?void 0:h.textContent)==null?void 0:m.toLowerCase())||"",a=[...t.querySelectorAll(".tag")].map(g=>{var f;return((f=g.textContent)==null?void 0:f.toLowerCase())||""}).join(" ");return r===e||d.includes(e)||c.includes(e)||a.includes(e)})(),o=N&&t.dataset.status==="done"&&ve(t.dataset.completedAt||"");t.style.display=s&&!o?"":"none"}),document.querySelectorAll(".column").forEach(t=>{const s=t.querySelectorAll(".card"),o=[...s].filter(a=>a.style.display!=="none").length,r=s.length,d=Number.parseInt(t.dataset.totalCount||`${r}`,10)||r,c=t.querySelector(".count");c&&(c.textContent=n||d!==r?`${o}/${d}`:`${d}`)})):C==="list"?(document.querySelectorAll("#list-view tbody tr").forEach(t=>{const s=!e||(()=>{var i,u,h,m;const r=t.dataset.id||"",d=((u=(i=t.querySelector(".col-title"))==null?void 0:i.textContent)==null?void 0:u.toLowerCase())||"",c=((m=(h=t.cells[5])==null?void 0:h.textContent)==null?void 0:m.toLowerCase())||"",a=[...t.querySelectorAll(".tag")].map(g=>{var f;return((f=g.textContent)==null?void 0:f.toLowerCase())||""}).join(" ");return r===e||d.includes(e)||c.includes(e)||a.includes(e)})(),o=N&&t.classList.contains("status-done")&&ve(t.dataset.completedAt||"");t.style.display=s&&!o?"":"none"}),document.querySelectorAll("#list-view .list-card").forEach(t=>{const s=!e||(()=>{var i,u,h;const r=t.dataset.id||"",d=((u=(i=t.querySelector(".list-card-title"))==null?void 0:i.textContent)==null?void 0:u.toLowerCase())||"",c=((h=t.dataset.project)==null?void 0:h.toLowerCase())||"",a=[...t.querySelectorAll(".tag")].map(m=>{var g;return((g=m.textContent)==null?void 0:g.toLowerCase())||""}).join(" ");return r===e||d.includes(e)||c.includes(e)||a.includes(e)})(),o=N&&t.classList.contains("status-done")&&ve(t.dataset.completedAt||"");t.style.display=s&&!o?"":"none"})):(document.querySelectorAll("#chronicle-view .chronicle-event").forEach(t=>{const s=!e||(()=>{var a,i,u;const r=t.dataset.id||"",d=((i=(a=t.querySelector(".chronicle-task-link"))==null?void 0:a.textContent)==null?void 0:i.toLowerCase())||"",c=((u=t.dataset.project)==null?void 0:u.toLowerCase())||"";return r===e||d.includes(e)||c.includes(e)})(),o=N&&ve(t.dataset.completedAt||"");t.style.display=s&&!o?"":"none"}),document.querySelectorAll("#chronicle-view .chronicle-group").forEach(t=>{const s=[...t.querySelectorAll(".chronicle-event")].filter(o=>o.style.display!=="none").length;t.style.display=s>0?"":"none"}))}function Be(e){if(!e||e==="null")return[];try{const n=JSON.parse(e);return Array.isArray(n)?n:[]}catch{return[]}}function gn(e){const n=new Date(e+"Z"),s=new Date().getTime()-n.getTime(),o=Math.floor(s/864e5);return o===0?"today":o===1?"yesterday":o<7?`${o}d ago`:o<30?`${Math.floor(o/7)}w ago`:e.slice(0,10)}function Y(e){if(!e||e==="null")return[];try{const n=JSON.parse(e);return Array.isArray(n)?n:[]}catch{return[]}}function vn(e){var M,l;const n=Ve(e.priority),t=n?`<span class="badge ${n}">${e.priority}</span>`:"",s=e.completed_at?`<span class="badge date">${e.completed_at.slice(0,10)}</span>`:e.created_at?`<span class="badge created">${gn(e.created_at)}</span>`:"",o=!E&&e.project?`<span class="badge project">${e.project}</span>`:"",r=Wt[e.status],d=r?`<span class="badge status-${e.status}">${r}</span>`:"",c=`<span class="badge level-${e.level}">L${e.level}</span>`,a=e.current_agent?`<span class="badge agent-tag">${e.current_agent}</span>`:"",i=e.last_review_status?[]:Y(e.review_comments),u=e.last_review_status||(i.length>0?(M=i[i.length-1])==null?void 0:M.status:null),h=u?`<span class="badge ${u==="approved"?"review-approved":"review-changes"}">${u==="approved"?"Approved":"Changes Req."}</span>`:e.status==="impl_review"?'<span class="badge review-pending">Awaiting Review</span>':"",m=e.last_plan_review_status?[]:Y(e.plan_review_comments),g=e.last_plan_review_status||(m.length>0?(l=m[m.length-1])==null?void 0:l.status:null),f=g?`<span class="badge ${g==="approved"?"review-approved":"review-changes"}">${g==="approved"?"Plan OK":"Plan Changes"}</span>`:e.status==="plan_review"?'<span class="badge review-pending">Plan Review</span>':"",$=Be(e.tags).map(p=>`<span class="tag">${p}</span>`).join(""),b=e.note_count??Y(e.notes).length,R=b>0?`<span class="badge notes-count" title="${b} note(s)">💬 ${b}</span>`:"",S=hn(e.level,e.status).map(p=>`<option value="${p}">${Ne(p)}</option>`).join(""),x=S?`
  ...
  `}function yn(e,n,t,s,o=s.length){const r=mn(e),d=Rt(s,e).map(vn).join(""),c=e==="todo"?'<button class="add-card-btn" id="add-card-btn" title="Add card">+</button>':"",a=o!==s.length?`${s.length}/${o}`:`${o}`;return`
  ...
  `}const bn=/```[\s\S]*?```/g,$n=/```\w*\n?/,vt=/```$/,yt=/^```mermaid\s*\n?/,wn=/\*\*(.+?)\*\*/g,En=/`([^`]+)`/g,Ln=/^\x00CB(\d+)\x00$/,In=/^### (.+)$/,Cn=/^## (.+)$/,Sn=/^# (.+)$/,jn=/^[-*]\s+(.+)$/,_n=/^\d+\.\s+(.+)$/,bt=/^\|(.+)\|$/,$t=/^\|[\s:-]+\|$/;let Bn=0;function Ie(e){const n=[];let t=e.replace(bn,i=>{if(yt.test(i)){const u=i.replace(yt,"").replace(vt,"").trim(),h=`mermaid-${++Bn}`;n.push(`<pre class="mermaid" id="${h}">${u}</pre>`)}else{const u=i.replace($n,"").replace(vt,"");n.push(`<pre><code>${u}</code></pre>`)}return`\0CB${n.length-1}\0`});t=t.replace(/</g,"&lt;"),t=t.replace(wn,"<strong>$1</strong>").replace(En,"<code>$1</code>");const s=t.split(`
`),o=[];let r=!1,d=!1;function c(){r&&(o.push("</ul>"),r=!1),d&&(o.push("</ol>"),d=!1)}let a=0;for(;a<s.length;){const i=s[a].trim(),u=i.match(Ln);if(u){c(),o.push(n[parseInt(u[1])]),a++;continue}if(bt.test(i)){c();const b=[];for(;a<s.length&&bt.test(s[a].trim());)b.push(s[a].trim()),a++;if(b.length>=2){const R=$t.test(b[1]),S=R?b[0]:null,x=R?2:0;let H='<table class="md-table">';if(S){const U=S.slice(1,-1).split("|").map(M=>M.trim());H+="<thead><tr>"+U.map(M=>`<th>${M}</th>`).join("")+"</tr></thead>"}H+="<tbody>";for(let U=x;U<b.length;U++){if($t.test(b[U]))continue;const M=b[U].slice(1,-1).split("|").map(l=>l.trim());H+="<tr>"+M.map(l=>`<td>${l}</td>`).join("")+"</tr>"}H+="</tbody></table>",o.push(H)}else o.push(`<p>${b[0]}</p>`);continue}const h=i.match(In);if(h){c(),o.push(`<h3>${h[1]}</h3>`),a++;continue}const m=i.match(Cn);if(m){c(),o.push(`<h2>${m[1]}</h2>`),a++;continue}const g=i.match(Sn);if(g){c(),o.push(`<h1>${g[1]}</h1>`),a++;continue}const f=i.match(jn);if(f){d&&(o.push("</ol>"),d=!1),r||(o.push("<ul>"),r=!0),o.push(`<li>${f[1]}</li>`),a++;continue}const $=i.match(_n);if($){r&&(o.push("</ul>"),r=!1),d||(o.push("<ol>"),d=!0),o.push(`<li>${$[1]}</li>`),a++;continue}c(),i===""?o.push(""):o.push(`<p>${i}</p>`),a++}return c(),o.join(`
`)}async function Tn(e){const n=window.__mermaid;if(!n)return;const t=e.querySelectorAll("pre.mermaid");if(t.length!==0)try{await n.run({nodes:t})}catch(s){console.warn("Mermaid render failed:",s)}}function Pe(e,n,t,s,o){if(!s&&!o)return"";const r=s?Ie(s):'<span class="phase-empty">Not yet documented</span>';return`
  ...
  `}).join("")}async function kn(e,n=1920,t=.82){return new Promise((s,o)=>{const r=new Image,d=URL.createObjectURL(e);r.onload=()=>{URL.revokeObjectURL(d);let{width:c,height:a}=r;(c>n||a>n)&&(c>a?(a=Math.round(a*n/c),c=n):(c=Math.round(c*n/a),a=n));const i=document.createElement("canvas");i.width=c,i.height=a,i.getContext("2d").drawImage(r,0,0,c,a),s(i.toDataURL("image/jpeg",t))},r.onerror=()=>{URL.revokeObjectURL(d),o(new Error("Image load failed"))},r.src=d})}async function Oe(e,n,t){var s,o;for(const r of Array.from(n)){if(!r.type.startsWith("image/"))continue;let d;try{d=await kn(r)}catch{d=await new Promise(u=>{const h=new FileReader;h.onload=()=>u(h.result),h.readAsDataURL(r)})}const c=(o=(s=r.name.match(/\.[^.]+$/))==null?void 0:s[0])==null?void 0:o.toLowerCase(),a=c===".jpg"||c===".jpeg"||c===".png"||c===".webp"||c===".gif"||c===".svg"?r.name:r.name.replace(/\.[^.]+$/,"")+".jpg",i=await _(`/api/task/${e}/attachment?project=${encodeURIComponent(t)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({filename:a,data:d})});if(!i.ok){const u=await i.json().catch(()=>({}));Ae(u.error||`Upload failed (${i.status})`);return}}W(e,t)}async function W(e,n){var r;const t=document.getElementById("modal-overlay"),s=document.getElementById("modal-content");s.innerHTML='<div style="color:#94a3b8">Loading...</div>',t.classList.remove("hidden"),J();try{const d=n?`?project=${encodeURIComponent(n)}`:"",a=await(await _(`/api/task/${e}${d}`)).json(),i=document.querySelector(`.card[data-id="${a.id}"][data-project="${CSS.escape(a.project)}"]`);i&&i.dataset.status!==a.status&&(at(),k());const u=Be(a.tags),h=u.length?`<div class="modal-tags">${u.map(v=>`<span class="tag">${v}</span>`).join("")}</div>`:"",m=[`<strong>Project:</strong> ${a.project}`,`<strong>Status:</strong> ${a.status}`,`<strong>Priority:</strong> ${a.priority}`,`<strong>Created:</strong> ${((r=a.created_at)==null?void 0:r.slice(0,10))||"-"}`,a.started_at?`<strong>Started:</strong> ${a.started_at.slice(0,10)}`:"",a.planned_at?`<strong>Planned:</strong> ${a.planned_at.slice(0,10)}`:"",a.reviewed_at?`<strong>Reviewed:</strong> ${a.reviewed_at.slice(0,10)}`:"",a.tested_at?`<strong>Tested:</strong> ${a.tested_at.slice(0,10)}`:"",a.completed_at?`<strong>Completed:</strong> ${a.completed_at.slice(0,10)}`:""].filter(Boolean).join(" &nbsp;|&nbsp; "),g={1:{labels:["Req","Impl","Done"],statuses:["todo","impl","done"]},2:{labels:["Req","Plan","Impl","Review","Done"],statuses:["todo","plan","impl","impl_review","done"]},3:{labels:["Req","Plan","Plan Rev","Impl","Impl Rev","Test","Done"],statuses:["todo","plan","plan_review","impl","impl_review","test","done"]}},f=g[a.level]||g[3],$=Math.max(0,f.statuses.indexOf(a.status)),b=`
    ...
    `,R=Y(a.attachments),S=R.length>0?`<div class="attachments-grid">${R.map(v=>`<div class="attachment-thumb" data-stored="${v.storedName}">
          ...
          </div>`).join("")}</div>`:"",x=a.description?Ie(a.description):'<span class="phase-empty">Not yet documented</span>',H=[1,2,3].map(v=>`<option value="${v}" ${v===a.level?"selected":""}>L${v}</option>`).join(""),U=`
    ...
    `,M=Pe("Plan","🗺️","phase-plan",a.plan,$===1&&!a.plan);let l="";a.decision_log&&(l=Pe("Decision Log","🧭","phase-decision-log",a.decision_log,!1));let p="";a.done_when&&(p=Pe("Done When","🎯","phase-done-when",a.done_when,!1));const y=Y(a.plan_review_comments),L=wt(y);let P="";(L||$===2)&&(P=`
      ...
      `);const O=Pe("Implementation","🔨","phase-impl",a.implementation_notes,$===3&&!a.implementation_notes),q=Y(a.review_comments),Z=wt(q);let xe="";(Z||$===4)&&(xe=`
      ...
      `);const Je=Y(a.test_results),he=An(Je);let ae="";(he||$===5)&&(ae=`
      ...
      `);const se=Y(a.agent_log);let rt="";if(se.length>0){let v=function(I){if(!I)return{name:"",model:null};const ge=I.toLowerCase();for(const Ge of w){const oe=ge.lastIndexOf(Ge);if(oe>0){let ie=oe;for(;ie>0&&(I[ie-1]==="-"||I[ie-1]==="_");)ie--;return{name:I.slice(0,ie),model:I.slice(oe)}}}return{name:I,model:null}};var o=v;const w=["opus","sonnet","haiku","gemini","copilot","gpt"],j=se.map(I=>{var ht;const{name:ge,model:Ge}=v(I.agent||""),oe=I.model||Ge,ie=oe?`<span class="badge model-tag model-${oe.toLowerCase()}">${oe}</span>`:"";return`
      ...
      `}const ct=Y(a.notes),Mt=ct.map(v=>{var w;return`
    ...
    `,Tn(s),s.querySelectorAll(".phase-expand-btn").forEach(v=>{v.addEventListener("click",w=>{w.stopPropagation();const j=v.closest(".lifecycle-phase");j==null||j.requestFullscreen().catch(()=>{})})});const lt=document.getElementById("level-select");lt.addEventListener("change",async()=>{const v=parseInt(lt.value);await _(`/api/task/${e}?project=${encodeURIComponent(a.project)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({level:v})}),z(a.project),W(e,a.project)}),document.getElementById("delete-task-btn").addEventListener("click",async()=>{confirm(`Delete card #${a.id} "${a.title}"?`)&&(await _(`/api/task/${e}?project=${encodeURIComponent(a.project)}`,{method:"DELETE"}),z(a.project),document.getElementById("modal-overlay").classList.add("hidden"),k())});const Ht=document.getElementById("req-edit-btn"),dt=document.getElementById("req-body-view"),pt=document.getElementById("req-body-edit"),Ye=document.getElementById("req-textarea"),ut=document.getElementById("req-save-btn"),Ut=document.getElementById("req-cancel-btn");Ht.addEventListener("click",()=>{dt.classList.add("hidden"),pt.classList.remove("hidden"),Ye.focus()}),Ut.addEventListener("click",()=>{Ye.value=a.description||"",pt.classList.add("hidden"),dt.classList.remove("hidden")}),ut.addEventListener("click",async()=>{const v=Ye.value;ut.textContent="Saving...",await _(`/api/task/${e}?project=${encodeURIComponent(a.project)}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({description:v})}),z(a.project),W(e,a.project)});const X=document.getElementById("attachment-drop-zone"),fe=document.getElementById("attachment-input");X&&fe&&(X.addEventListener("click",()=>fe.click()),X.addEventListener("dragover",v=>{v.preventDefault(),X.classList.add("drop-active")}),X.addEventListener("dragleave",()=>{X.classList.remove("drop-active")}),X.addEventListener("drop",async v=>{var j;v.preventDefault(),X.classList.remove("drop-active");const w=(j=v.dataTransfer)==null?void 0:j.files;w&&await Oe(e,w,a.project)}),fe.addEventListener("change",async()=>{fe.files&&await Oe(e,fe.files,a.project)})),s.addEventListener("paste",async v=>{var j;const w=Array.from(((j=v.clipboardData)==null?void 0:j.files)??[]).filter(I=>I.type.startsWith("image/"));w.length!==0&&(v.preventDefault(),await Oe(e,w,a.project))}),s.querySelectorAll(".attachment-remove").forEach(v=>{v.addEventListener("click",async w=>{w.stopPropagation();const j=v,I=j.dataset.id,ge=j.dataset.name;await _(`/api/task/${I}/attachment/${encodeURIComponent(ge)}?project=${encodeURIComponent(a.project)}`,{method:"DELETE"}),W(e,a.project)})});const Nt=document.getElementById("note-form"),mt=document.getElementById("note-input");Nt.addEventListener("submit",async v=>{v.preventDefault();const w=mt.value.trim();w&&(mt.disabled=!0,await _(`/api/task/${e}/note?project=${encodeURIComponent(a.project)}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:w})}),z(a.project),W(e,a.project))}),s.querySelectorAll(".note-delete").forEach(v=>{v.addEventListener("click",async w=>{w.stopPropagation();const j=v.dataset.noteId;await _(`/api/task/${e}/note/${j}?project=${encodeURIComponent(a.project)}`,{method:"DELETE"}),z(a.project),W(e,a.project)})})}catch{s.innerHTML='<div style="color:#ef4444">Failed to load</div>'}}function xt(e){if(!e)return new Date(NaN);let n=e.replace(" ","T");return n.length===10?n+="T00:00:00Z":/Z$|[+-]\d{2}:\d{2}$/.test(n)||(n+="Z"),new Date(n)}function Rn(e){const n=xt(e);if(isNaN(n.getTime()))return"Unknown";const t=n.getUTCDay()||7;n.setUTCDate(n.getUTCDate()+4-t);const s=new Date(Date.UTC(n.getUTCFullYear(),0,1)),o=Math.ceil(((n.getTime()-s.getTime())/864e5+1)/7);return`${n.getUTCFullYear()}-W${String(o).padStart(2,"0")}`}function xn(e){const n=xt(e);if(isNaN(n.getTime()))return e.slice(0,10)||"—";const t=n.toLocaleDateString(void 0,{month:"short",day:"numeric",timeZone:"UTC"}),s=n.toLocaleTimeString(void 0,{hour:"2-digit",minute:"2-digit",timeZone:"UTC",hour12:!1});return`${t}
${s}`}function Pn(e){const n=Ve(e.priority),t=!E&&e.project?`<span class="badge project">${e.project}</span>`:"",s=n?`<span class="badge ${n}">${e.priority}</span>`:"",o=`<span class="badge status-${e.status}">${Ne(e.status)}</span>`;return`
    ...
    </div>`}function qn(e){var u,h;const n=Ve(e.priority),t=n?`<span class="badge ${n}">${e.priority}</span>`:"",s=!E&&e.project?`<span class="badge project">${e.project}</span>`:"",o=`<span class="badge status-${e.status}">${Ne(e.status)}</span>`,r=`<span class="badge level-${e.level}">L${e.level}</span>`,d=((u=e.created_at)==null?void 0:u.slice(0,10))||"",c=((h=e.completed_at)==null?void 0:h.slice(0,10))||"—",i=Be(e.tags).map(m=>`<span class="tag">${m}</span>`).join("");return`
  ...
  `}async function Pt(){const e=document.getElementById("chronicle-view");try{const n=await me("full");Ke(n.projects),Ce();const t=[];for(const d of G)for(const c of n[d.key])t.push(c);const s=t.filter(d=>!!d.completed_at).sort((d,c)=>c.completed_at.localeCompare(d.completed_at)),o=new Map;for(const d of s){const c=Rn(d.completed_at);o.has(c)||o.set(c,[]),o.get(c).push(d)}if(o.size===0){e.innerHTML=`
        <div style="display:flex;align-items:center;justify-content:center;color:#64748b;font-size:0.9rem;padding:64px">
          No completed tasks yet
        </div>`;return}const r=[...o.entries()].map(([d,c])=>{const a=c.map(Pn).join("");return`
        <div class="chronicle-group">
          <div class="chronicle-week-header">${d}</div>
          <div class="chronicle-events">${a}</div>
        </div>`}).join("");e.innerHTML=`<div class="chronicle-timeline">${r}</div>`,e.querySelectorAll(".chronicle-task-link").forEach(d=>{d.addEventListener("click",c=>{c.stopPropagation();const a=parseInt(d.dataset.id),i=d.dataset.project||void 0;W(a,i)})}),ot()}catch(n){console.error("loadChronicleView failed:",n),e.innerHTML=`
    ...
    <div class="graph-placeholder">
      ...
      <p>Loading graph&hellip;</p>
          ...
          ${O}`,x.style.display="block"}).linkColor(l=>t[l.tag.toLowerCase()]??"#334155").linkWidth(l=>Math.min(1.5+(l.sharedCount-1)*.8,4)).d3AlphaDecay(.02).d3VelocityDecay(.3).warmupTicks(100).cooldownTime(5e3).width(e.offsetWidth||window.innerWidth).height(window.innerHeight-e.getBoundingClientRect().top).graphData({nodes:g,links:R});re=H;const U=new Set(g.flatMap(l=>l.tags.map(p=>p.toLowerCase()))),M=s.filter(l=>U.has(l)&&l in t).slice(0,16);if(M.length>0){const l=document.createElement("div");l.className="graph-legend",l.innerHTML=M.map(p=>`<div class="graph-legend-item"><span style="background:${t[p]}"></span>${p}</div>`).join(""),e.appendChild(l)}Q=new ResizeObserver(()=>{const l=e.offsetWidth,p=window.innerHeight-e.getBoundingClientRect().top;l>0&&p>0&&(e.style.height=`${p}px`,H.width(l).height(p))}),Q.observe(document.documentElement)}catch(c){console.error("loadGraphView failed:",c),e.innerHTML=`
      <div class="graph-placeholder">
        <p style="color:#ef4444;font-size:0.9rem">Failed to load graph</p>
      </div>`}}async function Te(){var n,t,s,o,r,d,c,a;const e=document.getElementById("board");try{const i=await me(Qe()?"full":"board");un(i),Ke(i.projects),gt?Ce():(gt=!0,Mn().then(()=>Ce())),e.innerHTML=G.map(g=>{var f;return yn(g.key,g.label,g.icon,i[g.key],((f=i.counts)==null?void 0:f[g.key])??i[g.key].length)}).join("");const u=((n=i.counts)==null?void 0:n.done)??i.done.length,h=i.total??(((t=i.counts)==null?void 0:t.todo)??i.todo.length)+(((s=i.counts)==null?void 0:s.plan)??i.plan.length)+(((o=i.counts)==null?void 0:o.plan_review)??i.plan_review.length)+(((r=i.counts)==null?void 0:r.impl)??i.impl.length)+(((d=i.counts)==null?void 0:d.impl_review)??i.impl_review.length)+(((c=i.counts)==null?void 0:c.test)??i.test.length)+(((a=i.counts)==null?void 0:a.done)??i.done.length);document.getElementById("count-summary").textContent=`${u}/${h} completed`,e.querySelectorAll(".card").forEach(g=>{g.addEventListener("click",f=>{if(f.target.closest(".card-interactive")){f.stopPropagation();return}const b=f.target.closest(".card-copy-btn");if(b){f.stopPropagation(),navigator.clipboard.writeText(b.dataset.copy).then(()=>{const x=b.textContent;b.textContent="✓",setTimeout(()=>{b.textContent=x},1e3)});return}const R=parseInt(g.dataset.id),S=g.dataset.project;W(R,S)})}),T||Hn(),On(),ze(),ot();const m=document.getElementById("add-card-btn");m&&m.addEventListener("click",g=>{g.stopPropagation(),document.getElementById("add-card-overlay").classList.remove("hidden"),J(),T||document.getElementById("add-title").focus()})}catch(i){console.error("loadBoard failed:",i),e.innerHTML=`
    ...
    `}}function On(){document.querySelectorAll("[data-column-toggle]").forEach(e=>{e.addEventListener("click",n=>{if(!T)return;n.stopPropagation();const t=e.dataset.columnToggle;if(!t)return;te.has(t)?te.delete(t):te.add(t),kt();const s=e.closest(".column"),o=te.has(t)||!!ue.trim();s&&s.setAttribute("data-mobile-expanded",String(o)),e.setAttribute("aria-expanded",String(o));const r=e.querySelector(".column-toggle-icon");r&&(r.textContent=o?"−":"+")})}),document.querySelectorAll(".mobile-status-select").forEach(e=>{e.addEventListener("click",n=>n.stopPropagation()),e.addEventListener("change",async n=>{n.stopPropagation();const t=e.value,s=parseInt(e.dataset.id||"",10),o=e.dataset.project||"",r=e.dataset.currentStatus||"";!s||!o||await fn({id:s,project:o,status:r},t)})})}async function Fe(){const e=document.getElementById("list-view");try{const n=await me("full");Ke(n.projects),Ce();const t=[];for(const a of G)for(const i of n[a.key])t.push(i);const s=ee==="default"?[...t].sort((a,i)=>i.id-a.id):Rt(t),o=s.length,r=s.filter(a=>a.status==="done").length;document.getElementById("count-summary").textContent=`${r}/${o} completed`;const d=s.map(a=>{var m,g;const i=Ve(a.priority),h=Be(a.tags).map(f=>`<span class="tag">${f}</span>`).join("");return`
      ...
      `}).join(""),c=s.map(qn).join("");e.innerHTML=`
