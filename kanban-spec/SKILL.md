...
curl -s "${AUTH_HEADER[@]}" -X POST "$BASE_URL/api/task?project=$PROJECT" \
  -H 'Content-Type: application/json' \
  -d "$(jq -n \
    --arg title "[title]" \
    --arg description "$DESCRIPTION" \
    --arg priority "[priority]" \
    --argjson level [level_number] \
    '{title: $title, description: $description, priority: $priority, level: $level, status: "todo"}')"
```
